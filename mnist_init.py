...
model = load_model('keras/mnist_model.h5')
...
while(True):
    ...
    _, gray = cv2.threshold(gray, 100, 255, cv2.THRESH_BINARY_INV) # need fixes
    #cv2.imshow("ROI", gray)
    ...
    example = np.squeeze(np.array(gray_small*255).astype("uint8"))
    cv2.imshow("ROI", example)
